...
int loadshader(char* filename, GLchar** ShaderSource, unsigned long* len)<br>{<br>   ifstream file;<br>	file.open(filename, ios::in);<br>   if(!file) return -1;<br>    <br>   len = getFileLength(file);<br>    <br>   if (len==0) return -2;   // Error: Empty File <br>    <br>   *ShaderSource = (GLubyte*) new char[len+1];<br>   if (*ShaderSource == 0) return -3;   // can't reserve memory<br>     <br>   *ShaderSource[len] = 0;  // len isn't always strlen cause some characters are stripped in ascii read...<br>                           // it is important to 0-terminate the real length later, len is just max possible value...<br>   unsigned int i=0;<br>   while (file.good())<br>   {<br>       *ShaderSource[i] = file.get();       // get character from file.<br>       if (!file.eof())<br>        i++;<br>   }<br>    <br>   *ShaderSource[i] = 0;  // 0 terminate it.<br>    <br>   file.close();<br>      <br>   return 0; // No Error<br>}
