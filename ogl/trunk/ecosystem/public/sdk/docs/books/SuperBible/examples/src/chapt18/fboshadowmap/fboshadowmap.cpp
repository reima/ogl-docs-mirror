...
// Demonstrates shadow mapping with FBOs
...
GLboolean useFBO = GL_FALSE;            // Use FBO renderbuffer for shadow map
...
GLint shadowWidth = 512;                // set based on window size
GLint shadowHeight = 512;
GLuint shadowTextureID;                 // shadow map name
...
void RenderScene(void)
{
    ...
    if (showShadowMap)
    {
        ...
        GLint showShadowWidth = (shadowWidth > windowWidth) ? windowWidth : shadowWidth;
        GLint showShadowHeight = (shadowHeight > windowHeight) ? windowHeight : shadowHeight;
        glBegin(GL_QUADS);
            glTexCoord2f(0.0f, 0.0f);
            glVertex2f(-1.0f, -1.0f);
            glTexCoord2f(1.0f, 0.0f);
            glVertex2f(((GLfloat)showShadowWidth/(GLfloat)windowWidth)*2.0f-1.0f, 
                       -1.0f);
            glTexCoord2f(1.0f, 1.0f);
            glVertex2f(((GLfloat)showShadowWidth/(GLfloat)windowWidth)*2.0f-1.0f, 
                       ((GLfloat)showShadowHeight/(GLfloat)windowHeight)*2.0f-1.0f);
...
void ChangeSize(int w, int h)
{
    ...
    if (useFBO)
    {
        ...
        shadowWidth = shadowHeight = maxTexSize;
    }
    else
    {
...
void ProcessMenu(int value)
{
    ...
    case 4:
        ...
        if (useFBO)
        {
            glutChangeToMenuEntry(4, "Toggle FBO usage (currently ON)", 4);
        }
        else
        {
            glutChangeToMenuEntry(4, "Toggle FBO usage (currently OFF)", 4);
        }
...
int main(int argc, char* argv[])
{
    ...
    glutAddMenuEntry("Toggle FBO usage (currently OFF)", 4);
