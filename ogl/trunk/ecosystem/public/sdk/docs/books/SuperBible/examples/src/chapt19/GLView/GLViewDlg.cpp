...
BOOL CGLViewDlg::OnInitDialog()
	...
	char cOutput[128];
	int nTabs[] = { 16, 40, 40+46, 64+46, 95+46, 159+46, 195+46, 222+46 };
	
	CDialog::OnInitDialog();
	m_ListBox.SetTabStops(8, nTabs);
	...
	for(int i = 1; i <= nFormatCount; i++)
		...
		// Flags
		wsprintf(cBuffer,"%08X\t",pfd.dwFlags);
...
void CGLViewDlg::OnBnClickedButton1()
{
    char pText[256];
    int i;
    HANDLE hFile;
    ...
    CFileDialog fileDlg (TRUE, "txt", "PixelFormats.txt",
    ...
    if( fileDlg.DoModal() == IDOK )
    {
        ...
        hFile=CreateFile(pathName, FILE_WRITE_DATA, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);

        for (i=0 ; ; i++)
        {

            if (LB_ERR==m_ListBox.GetText(i, pText))
                break;

            // write to file
            WriteFile(hFile, pText, strlen(pText), &foo, NULL);
            WriteFile(hFile, "\r\n", 2, &foo, NULL);
        }
...
void CGLViewDlg::OnLbnDblclkList()
{
    GLViewPixFormatDlg dlg;
    PIXELFORMATDESCRIPTOR pfd;		// Pixel format descriptor

    // Store format data into the class
    dlg.iFormatNumber = m_ListBox.GetCurSel() + 1;
    dlg.pFormatDesc   = &pfd;
	...
	HDC hDC = ::GetDC(this->m_hWnd);
	int nFormatCount = DescribePixelFormat(hDC, 1, 0, NULL);
    DescribePixelFormat(hDC, dlg.iFormatNumber, pfd.nSize, &pfd);

    // launch the child dialog
    dlg.DoModal();
}

void CGLViewDlg::OnBnClickedButton2()
{
    // Just like we double-clicked
    OnLbnDblclkList();
}
